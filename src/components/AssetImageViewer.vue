<template>
  <div class="asset-image-viewer" :class="`bg-${bgType}`">
    <el-image class="image" :src="src === null ? undefined : src || PNG_1PX" :infinite="false"
      :preview-src-list="src ? [src] : []" preview-teleported hide-on-click-modal fit="scale-down" @load="handleLoad" />
    ...
    <el-dropdown class="bg-select" placement="bottom-end" trigger="click">
      <el-button class="bg-select-btn" type="primary" link>배경<el-icon><i-el-arrow-down /></el-icon></el-button>
      <template #dropdown>
        <el-dropdown-menu>
          <el-dropdown-item @click="bgType = 'transparent'">투명</el-dropdown-item>
          <el-dropdown-item @click="bgType = 'black'">블랙</el-dropdown-item>
          <el-dropdown-item @click="bgType = 'white'">화이트</el-dropdown-item>
        </el-dropdown-menu>
      </template>
    </el-dropdown>
