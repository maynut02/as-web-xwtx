<template>
  <el-dropdown ref="dropdownRef" popper-class="menu-dropdown" trigger="click" placement="bottom-start"
    :popper-options="popoverOptions" @command="handleCommand" @visible-change="visible => (isOpen = visible)">
    <el-button class="menu-btn" :class="{ active: isOpen }" plain color="#444" @mouseenter="handleMouseEnter">{{
      config.name
    }}</el-button>
    <template #dropdown>
      <el-dropdown-menu>
        <el-dropdown-item v-for="(item, i) in config.items" :key="i" :command="i" :disabled="item.disabled?.()"
          :divided="item.divided" :icon="item.icon?.() || defaultIcon">{{ item.name }}</el-dropdown-item>
      </el-dropdown-menu>
    </template>
  </el-dropdown>
</template>
