<template>
  <div class="asset-list-table-wrapper">
    <div class="asset-list-table-header">
      <el-input v-model="searchInput" placeholder="검색" :prefix-icon="IElSearch" clearable />
    </div>
    <div class="asset-list-table-main" @dragover.capture.prevent @drop.capture.prevent="handleDropFiles">
      <vxe-table id="asset-list-table" ref="tableRef" class="asset-list-table" :data="searchedAssetInfos"
        :loading="store.isLoading" :border="true" size="mini" height="100%" header-cell-class-name="cursor-pointer"
        :row-class-name="({ row }) => ({
          ...
          " :row-config="{
            ...
            isCurrent: true,
            isHover: true,
          }" :column-config="{ resizable: true }" :menu-config="menuConfig" :keyboard-config="{ isArrow: true }"
        :checkbox-config="isMultiSelect ? { trigger: 'row', highlight: true, range: true, isShiftKey: true } : undefined
          " :custom-config="{ storage: { visible: true, resizable: true } }" :scroll-y="{ enabled: true }"
        show-overflow="title" show-header-overflow @menu-click="handleMenu" @header-cell-menu="handleHeaderCellMenu"
        @cell-menu="handleCellMenu" @cell-click="handleCellClick" @current-change="handleCurrentChange"
        @header-cell-click="handleHeaderCellClick" @checkbox-range-end="updateMultiSelectNum"
        @checkbox-all="updateMultiSelectNum">
        <vxe-column v-if="isMultiSelect" type="checkbox" :width="38" :resizable="false" fixed="left"
          header-class-name="cell-overflow-visible" class-name="cell-overflow-visible"></vxe-column>
        ...
        <template #empty>
          <el-text :style="{ fontSize: '30px', color: 'var(--el-color-info-light-3)' }">
            {{ store.assetInfos.length ? '데이터 없음' : '파일을 여기에 드롭하거나 "파일" 메뉴를 클릭하여 파일을 불러오세요' }}
          </el-text>
        </template>
      </vxe-table>
    </div>
    <div v-if="isMultiSelect" class="asset-list-table-footer wrap">
      ...
      <div class="footer-block actions">
        <el-button type="primary" size="small"
          ...
          @click="handleBatchExportSelected">내보내기</el-button>
        <el-button type="primary" size="small" @click="handleCancelMultiSelect">취소</el-button>
      </div>
    </div>
...
const menuConfig: VxeTablePropTypes.MenuConfig<AssetInfo> = reactive({
  header: {
    options: [
      [{ code: 'hideColumn', name: '현재 열 숨기기', prefixIcon: 'vxe-icon-eye-fill-close', disabled: false }],
      [
        { code: 'sortAsc', name: '내림차순 정렬', prefixIcon: '', visible: true, disabled: false },
        { code: 'sortDesc', name: '오름차순 정렬', prefixIcon: '', visible: true, disabled: false },
        { code: 'clearSort', name: '정렬 초기화', visible: true, disabled: false },
      ],
      [
        { code: 'resetResizable', name: '열 너비 초기화', prefixIcon: 'vxe-icon-ellipsis-h' },
        { code: 'resetVisible', name: '열 숨기기 초기화', prefixIcon: 'vxe-icon-eye-fill', disabled: false },
      ],
    ],
  },
  body: {
    options: [
      [
        { code: 'copy', name: '텍스트 복사', prefixIcon: 'vxe-icon-copy' },
        { code: 'export', name: '에셋 내보내기', prefixIcon: 'vxe-icon-save', disabled: false },
      ],
      [{ code: 'multiselect', name: '다중 선택', prefixIcon: 'vxe-icon-square-checked' }],
    ],
  },
